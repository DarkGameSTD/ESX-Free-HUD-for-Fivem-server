  ...
  methods: {
    ...
    getGiftTime() {
      ...
      const showRemaining = () => {
        ...
        if (distance < 0) {
			if (this.giftTime != false) {
				postNUI("EndGiftTimer")
				// console.log('gifted')
			}
			this.giftTime = false
			clearInterval(this.timer);
    ...
    messageHandler(event) {
		switch (event.data.action) {
			...
			case "CLOSE_HUD_SETTINGS":
				...
				let audiox = new Audio('./assets/sounds/click.ogg');
				audiox.play();
				audiox.volume = 0.10;
