            ...
            if (this.addSongUrl) {
				try{
					$.getJSON('https://noembed.com/embed?url=', { format: 'json', url: this.addSongUrl }, (data) => {
						...
						if (data.error){
							const titleNum = Math.floor(Math.random() * 100)
							const title = ("Song Number " + titleNum )
							let exist = this.songs.find((el) => el.url == this.addSongUrl)
							if(!exist){
								let nameExist = this.songs.find((el) => el.name == title)
								if(!nameExist){
									this.songs.push({
										name : title,
										url : this.addSongUrl, 
									...
									const titleNum = Math.floor(Math.random() * 99)
									const title = ("Song Number " + titleNum ) 
									this.songs.push({
										name : title,
										url : this.addSongUrl, 
