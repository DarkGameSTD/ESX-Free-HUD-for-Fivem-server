...
RegisterNetEvent('RV_HuD:LoadNitrous', function()
        ...
        if IsInVehicle and not IsThisModelABike(GetEntityModel(vehicle)) then
            if GetPedInVehicleSeat(veh, -1) == ped then
                ...
                if IsPedInAnyVehicle(ped) and GetPedInVehicleSeat(vehicle, -1) == ped then
                    ...
                    TriggerServerEvent('RV_HuD:server:LoadNitrous', Plate)
...
CreateThread(function()
    ...
    while true do
        ...
        if  CheckPlayerInVehicle() and DoesEntityExist(CurrentVehicle) then
            ...
            if VehicleNitrous[Plate] ~= nil then
                if VehicleNitrous[Plate].hasnitro then
                    ...
                    if IsDisabledControlJustPressed(0, Config.Keys[Config.NitroKey]) and not spam and GetPedInVehicleSeat(CurrentVehicle, -1) == PlayerPedId() then
                        ...
                        CreateThread(function()
                            while NitrousActivated do
                                if VehicleNitrous[Plate].level - 1 ~= 0 then
                                    TriggerServerEvent('RV_HuD:server:UpdateNitroLevel', Plate, (VehicleNitrous[Plate].level - 1))
                                else
                                    TriggerServerEvent('RV_HuD:server:UnloadNitrous', Plate)
                                    ...
                                    for index,_ in pairs(Fxs) do
                                        ...
                                        TriggerServerEvent('RV_HuD:server:StopSync', trim(GetVehicleNumberPlateText(CurrentVehicle)))
                                        Fxs[index] = nil
                                    end
                                end
                                nitroActive()
                    ...
                    if IsDisabledControlJustReleased(0, Config.Keys[Config.NitroKey]) and GetPedInVehicleSeat(CurrentVehicle, -1) == PlayerPedId() then
                        if NitrousActivated then
                            ...
                            for index,_ in pairs(Fxs) do
                                StopParticleFxLooped(Fxs[index], 1)
                                TriggerServerEvent('RV_HuD:server:StopSync', trim(GetVehicleNumberPlateText(veh)))
                                Fxs[index] = nil
                            end
                            StopScreenEffect("RaceTurbo")
    ...
    while true do
        if NitrousActivated then
            ...
            if veh ~= 0 and DoesEntityExist(veh) then
                TriggerServerEvent('RV_HuD:server:SyncFlames', VehToNet(veh))
...
RegisterNetEvent('RV_HuD:client:SyncFlames', function(netid, nosid)
...
RegisterNetEvent('RV_HuD:client:StopSync', function(plate)
...
RegisterNetEvent('RV_HuD:client:UpdateNitroLevel', function(Plate, level)
    VehicleNitrous[Plate].level = level
    setNitroValue(VehicleNitrous[Plate].level)

end)

RegisterNetEvent('RV_HuD:client:LoadNitrous', function(Plate)
...
RegisterNetEvent('RV_HuD:client:UnloadNitrous', function(Plate)
